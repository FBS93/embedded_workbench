    ...
    EDF_event_signal_t max_signal)
{
    ...
    {
        EMF_bitmask_clearAll(subs_list[idx].bitmask, EMF_UTILS_SIZEOF_ARRAY(subs_list[idx].bitmask));
    }
}
...
void EDF_activeObject_publish(const EDF_event_t *e)
{
    ...
    EAF_ASSERT_IN_CRITICAL_SECTION(e->sig <= EDF_framework.subscr_registry.max_signal);
    ...
    // Any subscribers?
    if (EMF_bitmask_isAnySet(subs_list.bitmask, EMF_UTILS_SIZEOF_ARRAY(subs_list.bitmask)))
    {
        prio = EMF_bitmask_findMax(subs_list.bitmask, EMF_UTILS_SIZEOF_ARRAY(subs_list.bitmask)); // Get highest-prio subscriber.
        ...
        while (EMF_bitmask_isAnySet(subs_list.bitmask, EMF_UTILS_SIZEOF_ARRAY(subs_list.bitmask)))
        {
            prio = EMF_bitmask_findMax(subs_list.bitmask, EMF_UTILS_SIZEOF_ARRAY(subs_list.bitmask)); // Get highest-prio subscriber.
            ao = EDF_framework.ao_registry[prio];                                                     // Get active object reference.
            EDF_activeObject_postFIFO(ao, e);
            EMF_bitmask_clearBit(subs_list.bitmask, prio); // Remove the handled subscriber.
        }
                                ...
                                EDF_event_signal_t sig)
{
    ...
    EAF_ASSERT_IN_CRITICAL_SECTION(sig <= EDF_framework.subscr_registry.max_signal);
...
void EDF_activeObject_subscribeAll(const EDF_activeObject_t *me)
{
    ...
    for (EDF_event_signal_t idx = (EDF_HSM_USER_SIGNAL - 1); idx < EDF_framework.subscr_registry.max_signal; ++idx)
    {
        EBF_CRITICAL_SECTION_ENTRY();
                             ...
                             EDF_eventQueue_t *e_queue)
{
    ...
    // Get event from defered queue.
    e = (EDF_event_t *)EDF_eventQueue_get(e_queue); // Typecast to discard const qualifier.

    // Queue shall not be empty when recalling.
    EAF_ASSERT(e != NULL);

    // Post recalled event.
    EDF_activeObject_postLIFO(me, e);
...
void EDF_activeObject_register(EDF_activeObject_t *me)
{
    ...
    for (uint8_t prio = (uint8_t)(me->prio - 1U);
         prio > 0U;
         --prio)
    {
...
const EDF_event_t *EDF_activeObject_get(EDF_activeObject_t *me)
{
    ...
    else
    {
        ...
        EAF_ASSERT_IN_CRITICAL_SECTION(me->e_queue.n_free == (me->e_queue.end + 1U));
    }
 ...
 *
 * The implementations of EDF_activeObject_postFIFO() and EDF_activeObject_postLIFO()
 * contain code that is largely duplicated from EDF_eventQueue_postFIFO() and
 * EDF_eventQueue_postLIFO(). This duplication is intentional and necessary.
 *
 ...
 * the edf_event_queue module) would require exiting and re-entering the critical section,
 * leaving a race condition window.
 *
 ...
 * EDF_CORE_WAIT_FOR_EVENT(me) to wait until an event arrives. This step must occur
 * *within the same critical section* to ensure atomicity and prevent
 * race conditions.
 *
 * Attempting to unify the functions (e.g. by calling into a shared function in
 * the edf_event_queue module) would require exiting and re-entering the critical section,
