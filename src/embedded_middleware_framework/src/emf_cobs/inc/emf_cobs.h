 ...
 * size.
 */
#define EMF_COBS_ENCODED_SIZE(size) ((size) +                  \
                                     EMF_COBS_OVERHEAD(size) + \
                                     EMF_COBS_PACKET_DELIMITER_SIZE)
 ...
 * @param[out] buff_out Pointer to encoded output buffer. The output buffer must be
 * at least len_in + (len_in / EMF_COBS_BLOCK_MAX) + 2 bytes to accommodate
 * worst-case COBS expansion.
 * @param[out] len_out Pointer to a variable that will be set to the encoded length.
 *
 ...
 * @param[out] len_out Pointer to a variable that will be set to the decoded length.
