 ...
 * Only entry actions are executed for each state on the path down to the descendant.
 * @startuml
 *  state Ancestor {
 *      state Mid {
 *          state Descendant
 ...
 * Exit actions are executed upward until (but not including) the ancestor target.
 * @startuml
 *  state Ancestor {
 *      state Mid {
 *          state Descendant
 ...
 * Transition between two states in different branches sharing a Lowest Common Ancestor (LCA).
 * Exit actions are executed up to (but not including) the LCA, then entry actions down to the target.
 * @startuml
 *  state LCA {
 *      state BranchA {
 *          state A1
 ...
 *
 * Key Definitions:
 * -Ancestor: A state that is higher in the hierarchy relative to another state (e.g., a parent, grandparent, etc.).
 * -Descendant: A state that is nested within another state (e.g., a child, grandchild, etc.).
 ...
 * -Self-transition: A transition where the source and target are the same state.
 * -Lowest Common Ancestor (LCA): The deepest (lowest) state in the hierarchy that is a common ancestor of both the source and the target states.
                                      ...
                                      int_fast8_t *x_idx)
{
    ...
    if (!t_found)
    {
        ...
        *e_idx = 0;
        e_path[*e_idx] = target;
        state_ret = DISPATCH_RESERVED_EVENT(e_path[*e_idx], EDF_HSM_EMPTY_SIGNAL); // Get target superstate.
        while ((state_ret == RET_SUPER) && (*e_idx < (EDF_HSM_MAX_DEPTH - 1)))     // -1: exclude top state.
        {
            (*e_idx)++;
            ...
            state_ret = DISPATCH_RESERVED_EVENT(me->temp_state, EDF_HSM_EMPTY_SIGNAL); // Get next ancestor.
        }
    }
    ...
    if (!t_found)
    {
        ...
        *x_idx = 0;
        x_path[*x_idx] = source;
        state_ret = DISPATCH_RESERVED_EVENT(x_path[*x_idx], EDF_HSM_EMPTY_SIGNAL); // Get source superstate.
        while ((state_ret == RET_SUPER) && (*x_idx < (EDF_HSM_MAX_DEPTH - 1)))     // -1: exclude top state.
        {
            (*x_idx)++;
    ...
    {
        /**
         * Case 4: Check is lowest common ancestor transition and trim paths accordingly.
         */
    ...
    int_fast8_t x_idx)
{
    ...
    while (DISPATCH_RESERVED_EVENT(aux_state, EDF_HSM_INIT_SIGNAL) == RET_TRANSITION)
    {
        e_idx = 0;
        ...
        while ((me->temp_state != aux_state) && (e_idx < (EDF_HSM_MAX_DEPTH - 1)))
        {
            ...
            (void)DISPATCH_RESERVED_EVENT(me->temp_state, EDF_HSM_EMPTY_SIGNAL); // Get next ancestor.
        }
...
void EDF_hsm_start(EDF_hsm_t *me, const EDF_event_t *e)
{
    ...
    EAF_ASSERT_IN_BLOCK(me->current_state == ((EDF_hsm_stateHandler_t)(&EDF_hsm_top)));
    EAF_ASSERT_BLOCK_END();
    ...
    (void)DISPATCH_RESERVED_EVENT(me->temp_state, EDF_HSM_EMPTY_SIGNAL); // Get parent of the initial transition target state.
    ...
    {
        e_path[e_idx] = me->temp_state;                                      // Store next ancestor state in the entry path.
        (void)DISPATCH_RESERVED_EVENT(me->temp_state, EDF_HSM_EMPTY_SIGNAL); // Get next ancestor until the EDF_hsm_top state is reached.
        e_idx++;
    }
    ...
    const EDF_event_t *e)
{
    ...
    for (int_fast8_t i = EDF_HSM_MAX_DEPTH; i > 0; i--)
    {
        ...
        // Invoke state handler (updates me->temp_state on RET_SUPER or RET_TRANSITION).
        ...
        {
            // Stop hierarchical propagation of the event.
            break;
        }
    }
    ...
    if (state_ret == RET_TRANSITION)
    {
        ...
        // Take the transition.
        EDF_hsm_getTransitionPath(me, t_source, t_target,
                                  e_path, &e_idx,
                                  x_path, &x_idx);
        EDF_hsm_executeTransition(me, e_path, e_idx,
                                  x_path, x_idx);
    }
}
